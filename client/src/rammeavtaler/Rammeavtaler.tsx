...
const Rammeavtaler = () => {
  ...
  const handleSearch = (value: string) => {
    setSearchTerm(value)
    const filteredAgreements = data?.content.filter((agreement) =>
      ...
      <div className='page__background-container'>
        ...
        <div className='page__content-container'>
          ...
          {data && data.totalPages && data.totalPages > 1 && searchTerm.length == 0 && (
            <Pagination page={pageState} onPageChange={(x) => setPageState(x)} count={data.totalPages} size='small'
                        prevNextTexts />
          )}
        </div>
      </div>
