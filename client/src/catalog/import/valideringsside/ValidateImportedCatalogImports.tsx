...
import { CatalogImport, DuplicateConflict } from 'utils/types/response-types'
  ...
  const importerDryrun = (supplier: string) => {
    ...
    importKatalogfil(upload, true, supplier)
      .then((response) => {
        ...
        setDuplicateConflicts(response.conflictList)
        setIsLoading(false)
      })
      .catch((error) => {
        setError(error)
        ...
        <div className={'import-products'}>
          <div className="content">
            ...
            <VStack gap="space-8">
              ...
              {(insertedList.length > 0 || updatedList.length > 0 || deactivatedList.length > 0) && (
                <BodyShort size="medium">
      ...
      <main>
        <div className="import-products">
          <div className="content">
            ...
            {!isLoading && (insertedList.length > 0 || updatedList.length > 0 || deactivatedList.length > 0) && (
              <VStack gap="space-16" minWidth="768px">
                <HStack gap="space-8">
                  <Button
                    className="fit-content"
                    size="medium"
                ...
                {duplicateConflicts.length > 0 && (
                  <VStack gap="space-8">
                    <Alert variant="warning">
                      Det oppsto {duplicateConflicts.length} duplikatkonflikter under importen
                    </Alert>
                    <Box background="default" padding="space-8">
                      <VStack gap="space-16">
                        <HStack justify="end">
                          <CopyButton
                            copyText={duplicateConflicts
                              .map(
                                (conflict) =>
                                  `LevArtNr: ${conflict.supplierRef} finnes allerede for denne leverandøren.\n` +
                                  `Importert produkt som fører til problem har hmsArtNr: ${conflict.conflictingHmsArtNr} og levArtNr: ${conflict.supplierRef}\n` +
                                  `Eksisterende produkt som fører til problem har hmsArtNr: ${conflict.hmsArtNr} og levArtNr: ${conflict.supplierRef}`
                              )
                              .join('\n\n')}
                          />
                        </HStack>
                        {duplicateConflicts.map((conflict, index) => (
                          <VStack>
                            <BodyShort>
                              LevArtNr: <b>{conflict.supplierRef}</b> finnes allerede for denne leverandøren.
                            </BodyShort>
                            <BodyShort>
                              Importert produkt som fører til problem har hmsArtNr: {conflict.hmsArtNr} og levArtNr:{' '}
                              {conflict.supplierRef}
                            </BodyShort>
                            <BodyShort>
                              Eksisterende produkt som fører til problem har hmsArtNr: {conflict.conflictingHmsArtNr} og
                              levArtNr: {conflict.supplierRef}
                            </BodyShort>
                          </VStack>
                        ))}
                      </VStack>
