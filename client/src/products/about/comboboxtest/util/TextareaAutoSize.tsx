...
const checkState = (prevState: State, newState: State, renders: React.MutableRefObject<number>) => {
...
const TextareaAutosize = forwardRef<HTMLTextAreaElement, TextareaAutosizeProps>(
  (
    { className, onChange, maxRows, minRows = 1, autoScrollbar, style, value, ...other }: TextareaAutosizeProps,
    ref,
  ) => {
    ...
    const mainStyle: React.CSSProperties = {
      ...
      // This prevents infinite rendering loop.
      overflow:
        state.overflow && !autoScrollbar && !inputRef.current?.style.height && !inputRef.current?.style.width
          ? "hidden"
          : undefined,
      ...style,
    };
...
function isEmpty(obj: State) {
  return (
    obj === undefined || obj === null || Object.keys(obj).length === 0 || (obj.outerHeightStyle === 0 && !obj.overflow)
  );
}
