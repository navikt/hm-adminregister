...
const CreateNews = () => {
  ...
  async function onSubmit(data: FormData) {
    const newNewsRelease: NewsRegistrationDTO = {
      ...
      text: textHtmlContent.replace("<p><br></p>", ""),
      published: data.publishedOn.toString(),
      expired: calculateExpiredDate(data.publishedOn, JSON.parse(data.duration)).toString(),
      // UNDER ARE TEMP VALS
      status: calculateStatus(data.publishedOn, JSON.parse(data.duration)),
      draftStatus: "DRAFT",
      created: data.publishedOn.toString(),
      updated: data.publishedOn.toString(),
      author: "a",
      createdBy: "a",
      updatedBy: "a",
      createdByUser: "a",
